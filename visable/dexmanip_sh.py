...
class DexManipSH_RH:
    ...
    def __init__(
        ...
        # video capture flags
        self.capture_video = bool(getattr(args, "record", 0))
        self.video_dir = getattr(args, "video_dir", "videos")
        self.video_fps = int(getattr(args, "fps", 30))
        self.video_suffix = getattr(args, "video_name_suffix", "")
        ...
        graphics_id = -1 if self.headless else args.graphics_device_id
        self.sim = self.gym.create_sim(
            args.compute_device_id, graphics_id, args.physics_engine, self.sim_params
        )
        ...
        # setup camera for video capture in headless as well
        self.cam_props = None
        self.cam_handles = None
    ...
    def _create_camera(self):
        # Create a single camera and a writer
        ...
        self.cam_props.horizontal_fov = 69.4
        cam_handle = self.gym.create_camera_sensor(self.envs[0], self.cam_props)
        # reasonable default location
        cam_pos = gymapi.Vec3(0.8, 0.0, 0.7)
        cam_target = gymapi.Vec3(-0.5, 0.0, 0.3)
        self.gym.set_camera_location(cam_handle, self.envs[0], cam_pos, cam_target)
        self.cam_handles = [cam_handle]

        ts = time()
        dt = datetime.fromtimestamp(float(ts))
        date_str = dt.strftime('%Y-%m-%d-%H-%M-%S')
        suffix = f"_{self.video_suffix}" if len(self.video_suffix) > 0 else ""
        out_dir = Path(self.video_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        self.video_path = out_dir / f"{date_str}{suffix}_rollout.mp4"
        self.video_writer = imageio.get_writer(self.video_path, fps=self.video_fps)

    def capture_frame(self):
        if not self.capture_video or self.cam_handles is None or self.video_writer is None:
            return
        self.gym.render_all_camera_sensors(self.sim)
        self.gym.start_access_image_tensors(self.sim)
        # single camera
        rgb = self.gym.get_camera_image(self.sim, self.envs[0], self.cam_handles[0], gymapi.IMAGE_COLOR)
        self.gym.end_access_image_tensors(self.sim)
        # convert and append
        rgb = rgb.reshape(self.cam_props.height, self.cam_props.width, 4)[..., :3]
        ...
        if self.headless == False:
            try:
                ...
                if self.viewer is None:
                    raise RuntimeError("gym.create_viewer returned None")
                ...
                print(f"[WARN] Failed to create viewer: {e}. Falling back to headless mode.")
                self.headless = True
                self.viewer = None
    ...
    def _create_obj_actor(self, env_ptr, i, current_asset):
        ...
        # 兼容 (T,4,4) 或 (4,4)
        if torch.is_tensor(obj_transf):
            if obj_transf.ndim == 3 and obj_transf.shape[-2:] == (4, 4):
                obj_transf = obj_transf[0]
        else:
            obj_transf = torch.tensor(obj_transf)
            if obj_transf.ndim == 3 and obj_transf.shape[-2:] == (4, 4):
                obj_transf = obj_transf[0]

        pose = gymapi.Transform()
        # 使用 Python float 避免 gymapi.Vec3 接受 tensor 导致 TypeError
        px = float(obj_transf[0, 3].item())
        py = float(obj_transf[1, 3].item())
        pz = float(obj_transf[2, 3].item())
        pose.p = gymapi.Vec3(px, py, pz)
        R = obj_transf[:3, :3]
        obj_aa = rotmat_to_aa(R)
        obj_aa_angle = float(torch.norm(obj_aa).item())
        if obj_aa_angle < 1e-8:
            ax_x, ax_y, ax_z = 1.0, 0.0, 0.0
            obj_aa_angle = 0.0
        else:
            obj_aa_axis = obj_aa / obj_aa_angle
            ax_x = float(obj_aa_axis[0].item())
            ax_y = float(obj_aa_axis[1].item())
            ax_z = float(obj_aa_axis[2].item())
        pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(ax_x, ax_y, ax_z), obj_aa_angle)
        ...
        for so_id, scene_obj in enumerate(scene_objs):
            ...
            pose = gymapi.Transform()
            sx = float(scene_obj_pose[0, 3].item())
            sy = float(scene_obj_pose[1, 3].item())
            sz = float(scene_obj_pose[2, 3].item())
            pose.p = gymapi.Vec3(sx, sy, sz)
